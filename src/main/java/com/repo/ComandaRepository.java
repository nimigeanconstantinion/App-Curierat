...
public class ComandaRepository {
    ...
    public void del(Comanda c) {
        if (isDeletable(c.getId())) {
            String sqlStr = String.format("delete from status where awb_colet in (select awb from colete where id_comanda=%d)", c.getId());
            executeStatement(sqlStr);
            sqlStr = String.format("delete from colete where id_comanda=%d", c.getId());
            executeStatement(sqlStr);
            sqlStr = String.format("delete from comenzi where id=%d", c.getId());
            executeStatement(sqlStr);

        } else {
            System.out.println("Comanda " + c.getId() + " nu mai poate fi stearsa din BD deoarece este in livrare!!!");
        }
    ...
    public Comanda getComandaActiva(int id_client) {
        ...
        if (getIdActiv(id_client) > 0) {
            ...
            try {
                ...
                ResultSet rs = statement.getResultSet();
                if (rs.next()) {
                    System.out.println("am gasit comanda activa");
                    ...
                    if (rs.getString(7) != null) {
                        c.setData_factura(LocalDateTime.parse(rs.getString(7), dtf));
                    } else {
    ...
    public boolean isDeletable(int id_Comanda) {
        String sqlStr = String.format("select count(*) from comenzi where id=%d and stare_comanda=%d", id_Comanda, 0);
        try {
            executeStatement(sqlStr);
            ResultSet rs = statement.getResultSet();
            if (rs.next()) {
